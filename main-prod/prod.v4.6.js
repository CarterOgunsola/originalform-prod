...
` &&
  ...
  const rv = {
    ...
    sv = () => {
      ...
      const i = () => {
          t.forEach((s) => {
            s && typeof s.revert == "function" && s.revert();
          }),
            (t = []),
            j.getAll().forEach((s) => s.kill());
        },
        n = () => {
          document.querySelectorAll("[split-text]").forEach((s) => {
            R.set(s, { autoAlpha: 1 });
            const a = s.getAttribute("split-text") || "lines",
              o =
                a === "lines"
                  ? ["lines"]
                  : a === "words"
